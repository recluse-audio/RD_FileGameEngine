...
// ---------------------------------------------------------------------------
// Resolve an image path: if the path has no file extension (asset folder),
// append the HiRes variant filename.
static std::string resolveImagePath(const std::string& sdFullPath)
{
    auto slash = sdFullPath.rfind('/');
    const std::string name = (slash == std::string::npos) ? sdFullPath : sdFullPath.substr(slash + 1);
    if (name.find('.') == std::string::npos)
        return sdFullPath + "/" + name + "_HiRes.png";
    return sdFullPath;
}
...
{
    drawPng(resolveImagePath(sdPath(path)));
}

void RaylibGraphicsRenderer::drawImage(const std::string& path, int x, int y, int w, int h)
{
    ...
    if (mCachedTexture.id <= 0) return;

    float texW  = (float)mCachedTexture.width;
    float texH  = (float)mCachedTexture.height;
    float scale = std::min((float)w / texW, (float)h / texH);
    ...
    Rectangle dst = { gx(drawX), gy(drawY), gp(drawW), gp(drawH) };
...
void RaylibGraphicsRenderer::drawText(const std::string& path, int x, int y)
{
    ...
    if (x == 0 && y > 0)
    {
        // Legacy overlay mode: semi-transparent backdrop, half-size text.
        DrawRectangle((int)gx(0), (int)gy(17),
                      (int)gp(320), (int)gp(240 - 17), {0, 0, 0, 200});
        drawMarkdown(full, 10, y, 0.5f, false);
    }
    else if (x == 0)
    {
        // Legacy full-screen scrollable mode.
        drawMarkdown(full, 10, 10, 1.0f, true);
    }
    else
    {
...
void RaylibGraphicsRenderer::drawFilledRect(int x, int y, int w, int h,
                                             int r, int g, int b, int alpha)
{
    DrawRectangle((int)gx(x), (int)gy(y), (int)gp(w), (int)gp(h),
...
void RaylibGraphicsRenderer::drawMarkdown(const std::string& fullPath,
                                           ...
                                           float textScale, bool applyScroll)
{
    std::ifstream file(fullPath);
    if (!file.is_open()) return;
    ...
    while (std::getline(file, line))
    {
        ...
        if (yPos + lineH > contentTop && yPos < contentBottom)
            DrawTextEx(mFont, display.c_str(), { gx(startX), yPos },
                       fontSize, 1.0f, color);
